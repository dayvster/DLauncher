...
{
  QString s = exec.trimmed();
  QStringList envAssignments;

  // Extract env assignments at the front (KEY=VALUE ...)
  QRegularExpression envRe(R"(([A-Za-z_][A-Za-z0-9_]*)=([^\s]+))");
  bool changed = true;
  while (changed)
  {
    changed = false;
    QRegularExpressionMatch m = envRe.match(s);
    if (m.hasMatch() && m.capturedStart(0) == 0)
    {
      envAssignments << (m.captured(1) + "=" + m.captured(2));
      s = s.mid(m.capturedLength(0)).trimmed();
      changed = true;
    }
  }

  // Expand desktop placeholders
  // %f - single file, %F - multiple files, %u - single URL, %U - multiple URLs
  // %c - translated name, %i - icon, %k - desktop file
  ...
  if (!appName.isEmpty())
    s.replace("%c", appName);
  if (!iconName.isEmpty())
    s.replace("%i", iconName);
  if (!desktopFilePath.isEmpty())
    s.replace("%k", desktopFilePath);

  // Handle file/URL placeholders
  if (!fileArgs.isEmpty())
  {
    QString joined = fileArgs.join(" ");
    s.replace("%F", joined);
    s.replace("%U", joined);
    // %f and %u -> first only
    s.replace("%f", fileArgs.first());
    s.replace("%u", fileArgs.first());
  }
  else
  {
    // remove leftover file placeholders
    s.remove("%F");
    s.remove("%U");
    s.remove("%f");
    s.remove("%u");
  ...
  // Now split the command safely
  QStringList parts = QProcess::splitCommand(s);
  if (parts.isEmpty())
